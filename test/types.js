...
describe("types", function () {
  it("should get Chișinău types - Place", function () {
    return index
      .getEntities({ language: "en", titles: ["Chișinău"], types: true })
  ...
  it("should get Barack Obama types - Person", function () {
    return index
      .getEntities({ language: "en", titles: ["Barack Obama"], types: true })
      .then(function (results) {
        assert.ok(results[0].types);
  ...
  it("should get Eurovision types - Event", function () {
    return index
      .getEntities({
        language: "en",
        titles: ["Eurovision Song Contest"],
        types: true
      })
      .then(function (results) {
        assert.ok(results[0].types);
  ...
  it("should identify fiction: Mickey Mouse", function () {
    return index
      .getEntities({ language: "ro", titles: ["Mickey Mouse"], types: true })
      .then(function (results) {
        assert.ok(results[0].types);
        assert.equal(
          true,
          results[0].types.indexOf("dbo:FictionalCharacter") > -1
        );
      });
  });
  ...
  it("place: US", function () {
    return index
      .getEntities({
        language: "ru",
        titles: ["Соединённые Штаты Америки"],
        types: true
      })
      .then(function (results) {
        assert.ok(results[0].types);
  ...
  it("place: Italy", function () {
    return index
      .getEntities({ language: "en", titles: ["Italy"], types: true })
      .then(function (results) {
        assert.ok(results[0].types);
        assert.equal(true, results[0].types.indexOf("dbo:Person") < 0);
      });
  ...
  it("place: Moscow", function () {
    return index
      .getEntities({ language: "en", titles: ["Moscow"], types: true })
      .then(function (results) {
        assert.ok(results[0].types);
        assert.equal(true, results[0].types.indexOf("dbo:Person") < 0);
      });
